...
Each BP and CP distributed by the OGC shall include a cover page with the statement as specified in https://portal.opengeospatial.org/public_ogc/directives/directives.php[Policy Directive 7].
...
Documents being submitted as Full standards shall use the https://portal.opengeospatial.org/files/?artifact_id=56777/[document formatting and content], in common use by the OGC at the time of submission, of standards submitted under the OGC Consensus Standards process. There is a detailed https://portal.opengeospatial.org/files/?artifact_id=3244/[guidance document] for using the template.
...
[lime]#For standards defining APIs where one or more API elements are proposed for removal in the revision, the API element deprecation process (<<deprecating-api-element>>) will also need to be followed to completion.#
